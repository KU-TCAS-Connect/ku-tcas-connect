...
from main_query_close_open_question_classify import query_type_classification
...
def hybrid_search_csv_documents(query, query_type, top_k=2):
    if query_type == "closed_ended":
        collection_name = vector_class.col_setting.collection_name["csv"]
    elif query_type == "open_ended":
        collection_name = vector_class.col_setting.collection_name["csv-header"]
    else:
...
def main_search_and_answer_csv(user_question, chat_history):
    ...
    query_type = query_type_classification(query)

    search_result = hybrid_search_csv_documents(query=query, query_type=query_type, top_k=2)
